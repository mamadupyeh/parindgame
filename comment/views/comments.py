...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        ), self.get_context_data(), request=self.request)
        ...
        if author_email:
            email = EmailMessage(
                'دیدگاه جدید',
                "دیدگاه جدیدی برای مقاله {} که شما نویسنده ان هستید ارسال شده است\n{}{}".format(article, current_site, reverse('blog:detail', kwargs={'slug': article.slug})), to=[author_email]
            )
            email.send()
        if user_email:
            email = EmailMessage(
                'دیدگاه دریافت شد',
                "دیدگاه شما درافت شد .بزودی به ان باسخ می دهیم", to=[user_email]
            )
            email.send()
        if parent_email:
            email = EmailMessage(
                "باسخ به دیدگاه شما",
                "باسخی به دیدگاه شما در مقاله {} ثبت شده است برای مشاهده روی لینک زیر کلیک کنید\n{}{}".format(article, current_site, reverse('blog:detail', kwargs={'slug': article.slug})), to=[parent_email]
            )
            email.send()
...
class UpdateComment(CanEditMixin, BaseCommentView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context['comment'] = self.comment
        self.data = render_to_string(
            'comment/comments/update_comment.html', context, request=self.request)
        return UTF8JsonResponse(self.json())
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            context['comment'] = self.comment
            self.data = render_to_string(
                'comment/comments/comment_content.html', context, request=self.request)
            return UTF8JsonResponse(self.json())
...
class DeleteComment(CanDeleteMixin, BaseCommentView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        context = self.get_context_data()
        self.data = render_to_string(
            'comment/comments/base.html', context, request=self.request)
        return UTF8JsonResponse(self.json())
